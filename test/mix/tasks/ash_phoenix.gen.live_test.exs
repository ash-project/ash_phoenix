...
defmodule Mix.Tasks.AshPhoenix.Gen.LiveTest do
  ...
  test "generate old phoenix live views from resource" do
    ...
    form_contents =
      ...
      defp assign_form(%{assigns: %{artist: artist}} = socket) do
        form =
          if artist do
            AshPhoenix.Form.for_update(artist, :update, as: "artist", actor: socket.assigns.current_user)
          else
            AshPhoenix.Form.for_create(AshPhoenix.Test.Artist, :create, as: "artist", actor: socket.assigns.current_user)
          end
  ...
  test "generate phoenix live views with no auth" do
    # no multitenancy
    send(self(), {:mix_shell_input, :yes?, false})
    # no actor
    send(self(), {:mix_shell_input, :yes?, false})
       ...
       defp assign_form(%{assigns: %{artist: artist}} = socket) do
         form =
           if artist do
             ...
             AshPhoenix.Form.for_create(AshPhoenix.Test.Artist, :create, as: "artist")
           end
    ...
    index_contents =
      ...
      {:ok,
       socket
        |> assign(:page_title, "Listing Artists")
        |> stream(:Artists, Ash.read!(AshPhoenix.Test.Artist))}
      end

      @impl true
      def handle_event("delete", %{"id" => id}, socket) do
      artist = Ash.get!(AshPhoenix.Test.Artist, id)
      Ash.destroy!(artist)
    ...
    show_contents =
      ...
      def mount(%{"id" => id}, _session, socket) do
        {:ok,
          socket
          |> assign(:page_title, "Show Artist")
          |> assign(:artist, Ash.get!(AshPhoenix.Test.Artist, id))}
      end
    ...
    assert Igniter.new()
           ...
           |> Igniter.Project.Module.move_files()
           |> assert_creates(form_path, form_contents)
           |> assert_creates(index_path, index_contents)
           |> assert_creates(show_path, show_contents)
  end

  test "generate phoenix live views with scope" do
    # using multitenancy with scope
      ...
      def render(assigns) do
       ...
       def mount(params, _session, socket) do
         artist =
           case params["id"] do
             nil -> nil
             id -> Ash.get!(AshPhoenix.Test.Artist, id, scope: socket.assigns.scope)
           end
       ...
       defp assign_form(%{assigns: %{artist: artist}} = socket) do
         form =
           if artist do
             AshPhoenix.Form.for_update(artist, :update, as: "artist", scope: socket.assigns.scope)
           else
             AshPhoenix.Form.for_create(AshPhoenix.Test.Artist, :create, as: "artist", scope: socket.assigns.scope)
           end
    ...
    index_contents =
      ...
      {:ok,
       socket
        |> assign(:page_title, "Listing Artists")
        |> stream(:Artists, Ash.read!(AshPhoenix.Test.Artist, scope: socket.assigns.scope))}
      end

      @impl true
      def handle_event("delete", %{"id" => id}, socket) do
      artist = Ash.get!(AshPhoenix.Test.Artist, id, scope: socket.assigns.scope)
      Ash.destroy!(artist, scope: socket.assigns.scope)
    ...
    show_contents =
      ...
      def mount(%{"id" => id}, _session, socket) do
        {:ok,
          socket
          |> assign(:page_title, "Show Artist")
          |> assign(:artist, Ash.get!(AshPhoenix.Test.Artist, id, scope: socket.assigns.scope))}
      end
  ...
  test "generate phoenix live views with tenant only" do
    # using multitenancy
    send(self(), {:mix_shell_input, :yes?, true})
    # not using scope
    send(self(), {:mix_shell_input, :yes?, false})
    # no actor
    send(self(), {:mix_shell_input, :yes?, false})
    # tenant name -> defaults to current_tenant
    send(self(), {:mix_shell_input, :prompt, ""})
      ...
      def render(assigns) do
       ...
       def mount(params, _session, socket) do
         artist =
           case params["id"] do
             nil -> nil
             id -> Ash.get!(AshPhoenix.Test.Artist, id, tenant: socket.assigns.current_tenant)
           end
       ...
       defp assign_form(%{assigns: %{artist: artist}} = socket) do
         form =
           if artist do
             AshPhoenix.Form.for_update(artist, :update, as: "artist", tenant: socket.assigns.current_tenant)
           else
             AshPhoenix.Form.for_create(AshPhoenix.Test.Artist, :create,
               as: "artist",
    ...
    index_contents =
      ...
      {:ok,
       socket
        ...
        |> stream(:Artists, Ash.read!(AshPhoenix.Test.Artist, tenant: socket.assigns[:current_tenant]))}
      end

      @impl true
      def handle_event("delete", %{"id" => id}, socket) do
      artist = Ash.get!(AshPhoenix.Test.Artist, id, tenant: socket.assigns.current_tenant)
      Ash.destroy!(artist, tenant: socket.assigns.current_tenant)
    ...
    show_contents =
      ...
      def mount(%{"id" => id}, _session, socket) do
        {:ok,
          socket
          |> assign(:page_title, "Show Artist")
          |> assign(:artist, Ash.get!(AshPhoenix.Test.Artist, id, tenant: socket.assigns.current_tenant))}
      end
  ...
  test "generate phoenix live views with actor and tenant" do
    # using multitenancy
    send(self(), {:mix_shell_input, :yes?, true})
    # not using scope
    send(self(), {:mix_shell_input, :yes?, false})
    # will name actor (truthy string)
    send(self(), {:mix_shell_input, :yes?, "y"})
    # actor name -> defaults to current_user
    send(self(), {:mix_shell_input, :prompt, ""})
    # tenant name -> defaults to current_tenant
    send(self(), {:mix_shell_input, :prompt, ""})
    ...
    form_contents =
      ...
      def render(assigns) do
       ...
       def mount(params, _session, socket) do
         artist =
           case params["id"] do
             nil -> nil
             id ->
               Ash.get!(AshPhoenix.Test.Artist, id,
                 actor: socket.assigns.current_user,
                 tenant: socket.assigns.current_tenant
               )
           end
       ...
       defp assign_form(%{assigns: %{artist: artist}} = socket) do
         form =
           if artist do
             AshPhoenix.Form.for_update(artist, :update, as: "artist", actor: socket.assigns.current_user, tenant: socket.assigns.current_tenant)
           else
             AshPhoenix.Form.for_create(AshPhoenix.Test.Artist, :create,
               as: "artist",
               actor: socket.assigns.current_user,
    ...
    index_contents =
      ...
      {:ok,
       socket
        ...
        |> stream(
          :Artists,
          Ash.read!(AshPhoenix.Test.Artist,
            actor: socket.assigns[:current_user],
            tenant: socket.assigns[:current_tenant]
          )
        )}
